...
class NeuronGroup:
    ...
    def step(self, I_ext):
        ...
        # fancy indexing — scatter writes are very slow on GPU for small tensors.
        in_ref = self.ref > 0
        self.V = torch.where(in_ref, self.V.new_full((), VR), self.V)
        self.ref.sub_(in_ref.to(dtype=torch.int32))

        fired = (self.V > VT) & (~in_ref)
        fired_f = fired.to(dtype=torch.float32)
        self.spk.copy_(fired)
        self.V = torch.where(fired, self.V.new_full((), VR), self.V)
        self.a.add_(fired_f * B_ADP)
        self.ref = torch.where(fired, self.ref.new_full((), TREF), self.ref)
        self.Y = torch.where(fired, self.Y.new_full((), 1.0), self.Y)
...
# ─────────────────────────────────────────────────────────────
    def __init__(self, area_sizes=None, n_gist=16,
                 ...
                 cls_clamp_gain=800.0, clamp_mode="set_I_R",
                 cls_force_spike_every: int = 1,
        ...
        if clamp_mode not in ("set_I_R", "add_clamp_gain", "overwrite_fired"):
            raise ValueError(
                f"clamp_mode must be 'set_I_R', 'add_clamp_gain', or 'overwrite_fired'; got {clamp_mode!r}"
            )
    ...
    def step(self, pixel_pA, class_label=None, class_clamp_gain: float | None = None):
        ...
        for l in range(self.L - 1):
            ...
            if (self.use_class_area and class_label is not None and (l + 1) == (self.L - 1)):
                if self.clamp_mode == "set_I_R":
                    I_R = self.label_clamp_current(class_label, class_clamp_gain) * scale
                elif self.clamp_mode == "add_clamp_gain":
                    I_R += self.label_clamp_current(class_label, class_clamp_gain) * scale
                else:  # overwrite_fired: zero out PC current; spikes injected manually below
                    I_R = torch.zeros_like(I_R)

            areas[l + 1].R.step(I_R)

            if (self.use_class_area and class_label is not None
                    and (l + 1) == (self.L - 1) and self.clamp_mode == "overwrite_fired"):
                top = areas[l + 1].R
                # clear any spikes/traces produced by the zero-current step
                top.spk.zero_()
                top.Y.zero_()
                # force the correct class neuron to spike every cls_force_spike_every steps
                if (self.cls_force_spike_every > 0
                        and (self._step_idx % self.cls_force_spike_every == 0)):
                    lbl = _to_tensor(class_label, device=self.device, dtype=torch.long).reshape(-1)
                    bidx = torch.arange(lbl.shape[0], device=self.device)
                    top.spk[bidx, lbl] = True
                    top.Y[bidx, lbl] = 1.0
                    top.V[bidx, lbl] = VR
                    top.a[bidx, lbl] += B_ADP
                    top.ref[bidx, lbl] = TREF
    ...
    def run_batch(self, pixel_pA, class_label=None, class_clamp_gain: float | None = None):
        ...
        R_acc  = {l: torch.zeros(B, self.area_sizes[l],  device=self.device) for l in range(self.L)}
        EP_acc = {l: torch.zeros(B, self.area_sizes[l],  device=self.device) for l in range(self.L - 1)}
        EN_acc = {l: torch.zeros(B, self.area_sizes[l],  device=self.device) for l in range(self.L - 1)}

        for t in range(n):
            self.step(pixel_pA, class_label=class_label, class_clamp_gain=class_clamp_gain)
            if t >= n - tw:
                for l in range(self.L):
                    R_acc[l].add_(self.areas[l].R.X)
                for l in range(self.L - 1):
                    EP_acc[l].add_(self.areas[l].EP.X)
                    EN_acc[l].add_(self.areas[l].EN.X)

        X_R  = {l: R_acc[l]  / tw for l in range(self.L)}
        X_EP = {l: EP_acc[l] / tw for l in range(self.L - 1)}
        X_EN = {l: EN_acc[l] / tw for l in range(self.L - 1)}
